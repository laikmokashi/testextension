...
import { AwsResourceListService, AwsResourceList, REMOTE_UserSession } from '../aws-resource-list.service';
  ...
  template: `
    <div class="card datatable-card">
      <searchable-datatable
        ...
        <ngx-datatable-column name="Region" [flexGrow]="120">
          <ng-template ngx-datatable-cell-template let-row="row">{{ row.spec?.region || '—' }}</ng-template>
        </ngx-datatable-column>

        <ngx-datatable-column name="Instances" [flexGrow]="100">
          <ng-template ngx-datatable-cell-template let-row="row">{{ row.result?.instances?.length ?? '—' }}</ng-template>
        </ngx-datatable-column>
...
export class ListAwsResourceListComponent implements OnInit {
  ...
  private readonly allRows = signal<AwsResourceList[]>([]);
  private readonly filterTerm = signal('');
  private readonly searchFields = ['name', 'status', 'spec.region'];
  ...
  private refresh(tenantChanged: boolean): void {
    if (tenantChanged) this.table()?.startLoading();
